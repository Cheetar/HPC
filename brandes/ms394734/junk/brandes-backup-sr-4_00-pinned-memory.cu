  ...
  for (source = 0; source < no_vertices; source++) {
    ...
    *cont = true;
    while (*cont) {
      ...
      *cont = false;
      HANDLE_ERROR(cudaMemcpyToSymbol(dev_cont, cont, sizeof(bool), cudaMemcpyHostToDevice));

      forward<<<BLOCKS,THREADS_PER_BLOCK>>>(dev_edges_x, dev_edges_y, sigma, dist, no_edges, level, dev_cont);

      level++;
      // Load cont value from device
      HANDLE_ERROR(cudaMemcpyFromSymbol(cont, dev_cont, sizeof(bool), cudaMemcpyDeviceToHost));
    }
